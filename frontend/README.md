...
«Демо на тестовых данных» заполняет оба комплекта файлами из эталона (6 + 7) и запускает
локальный mock (`src/api/mock.ts`) с id `demo-…`. Данные — `handoff/demo-analysis.json`,
приведённый к формату контракта (`src/api/fixtures/demo-analysis.json`). В шапке такого
анализа виден чип «Тестовые данные».
